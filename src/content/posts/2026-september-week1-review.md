...
![MVP 플랫폼의 계층별 구조. 접근 계층은 Nginx, 배포 실행 계층은 워커, 서비스 실행 계층은 Preview, 상태 저장 계층은 PostgreSQL로 구성된다. 사용자 포털 계층과 플랫폼 제어 계층, 변경 감지 계층의 Webhook 수신은 같은 API 프로세스에 있으며 Poller는 별도 프로세스다.](@/assets/images/2026-september-week1-review/mvp-platform-architecture.svg)

1. **접근 계층**: Nginx가 호스트 이름에 따라 사용자 포털·플랫폼 제어 계층의 API와 서비스 실행 계층의 Preview로 요청을 보낸다.
2. **변경 감지 계층**: Webhook으로 main 변경을 감지하고, Poller가 누락을 보완한다. 배포 작업은 상태 저장 계층의 PostgreSQL 큐에 저장한다.
3. **배포 실행 계층**: 워커가 작업을 가져와 해당 커밋을 빌드·테스트하고 서비스 실행 계층의 Preview에 배포한다. 단계별 결과는 상태 저장 계층에 기록한다.
