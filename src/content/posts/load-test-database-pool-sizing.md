...
| API                     | ASIS · 풀 조정 전 | TOBE · 풀 조정 후 |
| ----------------------- | ----------------: | ----------------: |
| 토큰 발급               |            20.78s |            28.28s |
| 출석 상태 조회(`today`) |            21.00s |            28.54s |
| 출석 확정               |            21.28s |            28.41s |
| 현황판 조회(`board`)    |            20.86s |            28.29s |
...
API별 p99입니다. 태스크당 1 vCPU·4GB는 동일합니다.

| API                     | ASIS · 태스크 2개 | TOBE · 태스크 4개 |
| ----------------------- | ----------------: | ----------------: |
| 토큰 발급               |            28.28s |            18.78s |
| 출석 상태 조회(`today`) |            28.54s |            19.26s |
| 출석 확정               |            28.41s |            19.43s |
| 현황판 조회(`board`)    |            28.29s |            18.96s |
...
API별 p99입니다. 태스크당 메모리는 4GB로 유지했습니다.

| API                     | ASIS · 1 vCPU·4개 | TOBE · 2 vCPU·8개 | 재측정 · 2 vCPU·5개 |
| ----------------------- | ----------------: | ----------------: | ------------------: |
| 토큰 발급               |            18.78s |             1.90s |             20.43ms |
| 출석 상태 조회(`today`) |            19.26s |             2.19s |              80.8ms |
| 출석 확정               |            19.43s |             2.57s |              55.9ms |
| 현황판 조회(`board`)    |            18.96s |             2.08s |             33.53ms |
