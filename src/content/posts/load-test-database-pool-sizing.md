...
| 항목                             | ASIS · 풀 조정 전 | TOBE · 풀 조정 후 |
| -------------------------------- | ----------------: | ----------------: |
| Master 획득 p99 · 구간 최대      |            1.25초 |             691ms |
| Replica 획득 p99 · 구간 최대     |            1.36초 |             383ms |
| SELECT 실행 평균 · 조회 트레이스 |           24.75ms |          106.84ms |
...
| 항목          | ASIS                   | TOBE                        |
| ------------- | ---------------------- | --------------------------- |
| API 응답 시간 | 기준 초과              | 검증한 부하에서 기준 충족   |
| Redis 호출    | 트레이스에서 지연 확인 | 비교 트레이스에서 지연 감소 |
