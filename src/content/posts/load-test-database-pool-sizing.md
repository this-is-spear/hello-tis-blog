...
이전에 측정한 DB CPU 사용률입니다. 측정일과 인스턴스 사양을 확인하지 못해, 이번 풀 조정은 커넥션 획득 시간을 기준으로 진행했습니다.
...
2 vCPU 구성에서 p99가 3초 이내로 줄었습니다. CPU 설정을 유지한 채 태스크 수를 줄여도 목표를 충족하는지 확인했습니다.
...
DB 풀 조정과 태스크·CPU 증설을 거쳐, 검증한 부하에서 **모든 API의 p99가 3초 이내**라는 목표에 도달했습니다.

| API                     | 초기 p99 | 최종 p99 | 단축(약) |
| ----------------------- | -------- | -------- | -------- |
