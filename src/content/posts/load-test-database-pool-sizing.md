...
- 테스트 태그·목표 부하·시간·VU: 2차와 동일

## 결과

### TOBE

| API                     | 2차 · ECS 2개 p99 | 3차 · ECS 4개 p99 |
| ----------------------- | ----------------: | ----------------: |
| 토큰 발급               |            28.28s |            18.78s |
...
- 완료 iteration: 28,885 → 66,558 → 76,963건
- 미시작 iteration: 61,116 → 23,443 → 13,038건
- HTTP 실패: 436 / 60,374 → 0 / 139,068 → 0 / 160,803건
- HTTP 응답 시간 평균: 15.06초 → 3.56초 → 5.34초
- HTTP 응답 시간 p50: 15.88초 → 379.87ms → 1.7초

완료 건수는 종료 대기까지 포함한 로그 합계입니다. 2차는 3분, 3차는 3분 16.9초에 종료됐습니다.

3차는 27초에 VU 5,000개에 도달했고, 모든 API의 p99가 3초 기준을 초과했습니다.
