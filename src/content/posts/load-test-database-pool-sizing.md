...
API별 p99를 약 **20.76 ~ 21.22초 단축**했고, 모든 API가 **3초 이내** 기준을 충족했습니다.

ASIS는 1 vCPU·태스크 2개·DB 풀 10, TOBE는 2 vCPU·태스크 5개 구성입니다.
...
튜닝하면서 CPU 상한과 DB 커넥션 상한을 함께 고려해 설정을 고민했습니다. 고려할 자원이 여러 개라면 **부하를 높일 때 먼저 상한에 도달하는 자원부터 조정하면 되겠다는 생각이 들었습니다.**

- CPU가 먼저 상한에 도달하면 CPU 증설이나 스케일 아웃을 검토합니다.
- DB 커넥션이 먼저 상한에 도달하면 태스크당 풀 크기와 커넥션 점유 시간을 줄일 수 있는지 검토합니다.

한쪽을 조정하면 다른 쪽이 먼저 한계에 도달할 수 있으므로, 같은 부하에서 다시 측정하며 설정을 맞추려 합니다.
