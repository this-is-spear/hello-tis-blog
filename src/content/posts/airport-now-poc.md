...
2026년 9월 10일 확인한 두 서비스의 접근 과정과 화면을 비교했습니다.
...
_2026년 9월 10일 배포 화면. 수집 지연으로 마지막 저장 정보가 표시된 상태입니다._
...
_2026년 9월 10일 캡처. 저장된 T1 단기주차장 지하 1층의 관측 이력입니다._
...
현재 배포한 PoC에서는 주차 상태와 변화, 항공편 연결, 주차장 비교, 관심 저장과 공유를 이용할 수 있습니다. 기획에서 정리한 세 상황을 실제 화면으로 옮겼고, 이후 분석을 위한 수집·저장 구조도 마련했습니다.

이 과정에서 기능을 고르는 기준을 얻었습니다. 정보를 찾는 과정을 나누니 검색과 설치 단계를 줄일 여지가 보였고, 일행에게 전달하는 과정을 나누니 항공편 경로를 추가할 이유가 생겼습니다. 구현에서는 배치 실패와 API 실패에 따른 조회 경로를 나눴고, 저장에서는 관측 횟수와 운항편 수를 구분했습니다.

PoC를 이어가며 필요한 정보를 빨리 찾는지, 같은 일정에서 다시 여는지, 공유받은 사람이 재검색 없이 확인하는지를 살펴보려고 합니다. 그 흐름을 확인한 뒤 보험 이벤트 연결을 검토할 생각입니다. 마중객처럼 보험과 목적이 다른 사용자도 있으므로, 기능 이용과 이벤트 참여를 함께 가정하지는 않으려 합니다.
